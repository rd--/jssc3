...
ScSynth can be compiled to Wasm, and run in an Html page.  Osc packets can be sent from the Js interpreter of the web browser to the synthesiser.  In this schema the Js interpreter of the browser performs the functions peformed by ScLang in ordinary use.
...
JsSc3 programs can easily be written as [rich text](http://www.bitsavers.org/pdf/xerox/alto/BravoCourse.pdf) documents.  The area containg this text is a simple editor, the controls above the text area are for formatting, and the controls below are for communicating with the Sc synthesiser.
...
Please note that, in this simple system, recalling a saved text _replaces_ the existing text, which is deleted and cannot be recovered.
...
Once loaded the status area will display _----_, indicating that the page is ready, after which the synthesiser can be started by clicking _Boot_.
...
To make programs simpler to select they can be set apart in a _</>_  paragraph by themselves.  The program below is an Sc2 example written by JMcC.  It makes bird like sounds.  When running, the status area will indicate that the program involves 108 unit generators.
...
The _notation format_ menu selects which notation programs are to be interpreted as.  At present the two notations are _.stc_ and _.js_.
...
Stc and Js notations are similar in many ways.  One significant difference is that _infix_ operators are replaced by functions, here _*_ is replaced by _mul_.  This is required because in Js operators are special forms and are not extensible.

The function syntax is also somewhat different, as can be seen in the program below, another Sc2 example.
