...
function stcPlay() {
	const stcText = sc.getSelectedText();
	console.log(`stcPlay: ${stcText}`);
	sc.stcToJs(stcText).then(
		(jsText) => playUgen(eval(jsText)),
	);
}
...
function insertMarkdown(text) {
	...
	document.getElementById('documentText').innerHTML = writer.render(
		reader.parse(text),
	);
}
...
function ugenHelp() {
	...
	if (name.length > 0) {
		...
		sc.fetchUtf8(url, { cache: 'no-cache' }).then(insertMarkdown);
	}
}
...
export function loadHelp() {
	sc.fetchUtf8('help/Essay/SuperScript.md', { cache: 'no-cache' }).then(
		insertMarkdown,
	);
}
