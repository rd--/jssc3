...
function fetch_url_and_then(url : string, responseType : XMLHttpRequestResponseType, proc : (x : any) => void) : void {
...
function handle_fetch_error(response : Response) : Response {
    if (!response.ok) {
...
function load_and_extract_and_then(fileName : string, typeString : string, extractFunc : ExtractPromise, processFunc : (x : any) => void) : void {
...
function load_utf8_and_then(fileName : string, processFunc : (x : string) => void) : void {
...
function load_json_and_then(fileName : string, processFunc : (x : object | []) => void) : void {
...
function load_arraybuffer_and_then(fileName : string, processFunc : (x : ArrayBuffer) => void) : void {
    load_and_extract_and_then(fileName, 'bytes/arraybuffer', r => r.arrayBuffer(), processFunc);
}

// Read text file and run proc on result.
function read_text_file_and_then(textFile : File, proc : (x : string) => void) : void {
...
function read_text_file_from_file_input_and_then(inputId : string, fileIndex : number, proc : (x : string) => void) : void {
...
function read_json_file_and_then(jsonFile : File , proc : ( x : object | []) => void) : void {
