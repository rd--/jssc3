...
function sc3_supercalc_on_change (_instance, _cell, col_index, row_index, cell_text) {
	const col_letter = column_index_to_letter(Number(col_index));
...
function sc3_supercalc_gen_cell_reader_bus_declaration(col_letter, row_number) {
	...
	return ('const ' + var_name + ' = sc.InFb(1, ' + String(bus_index) + ');');
}
...
export function sc3_supercalc_server_setup() {
	...
	sc3_supercalc_define_cell_variables(calc.num_col, calc.num_row);
	sc3_supercalc_create_and_init_cell_groups(calc.num_col, calc.num_row);
}
