...
export function dictionaryPut<T>(
	aDictionary: Dictionary<T>,
	aKey: string,
	aValue: T,
): void {
...
export function dictionaryHasKey<T>(
	aDictionary: Dictionary<T>,
	aKey: string,
): boolean {
...
export function dictionaryCopyAllFromTo<T>(
	sourceDictionary: Dictionary<T>,
	destinationDictionary: Dictionary<T>,
): void {
	Object.entries(sourceDictionary).forEach(([key, value]) =>
		destinationDictionary[key] = value
	);
}

// Find key at aDictionary that holds aValue.
export function dictionaryFindKeyOfValue<T>(
	aDictionary: Dictionary<T>,
	aValue: T,
): string | undefined {
...
export function dictionaryCopyKeys<T>(
	aDictionary: Dictionary<T>,
	keysArray: string[],
): Dictionary<T> {
