...
import { scSynthDefaultOptions, ScSynthOptions } from './scSynthOptions.ts';
import {
	c_setn1,
	d_recv,
	defaultScSynthStatus,
	g_freeAll,
	g_new,
	kAddToTail,
	m_dumpOsc,
	m_notify,
	m_parseStatusReplyInto,
	m_status,
	s_new,
	ScSynthStatus,
} from './serverCommand.ts';
	...
	whenConnected(activity: () => void) {
		switch (this.readyState) {
			...
			case ReadyState.Disconnected:
				console.log(
					'ScSynth.whenConnected: disconnected, start and schedule activity',
				);
				this.connect();
				setTimeout(() => this.whenConnected(activity), 1000);
				break;
			default:
				console.log(
					'ScSynth.whenConnected: unknown readyState',
					this.readyState,
				);
		}
	}
