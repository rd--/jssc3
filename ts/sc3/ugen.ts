...
export function localControlIndexCompare(
	i: LocalControl,
	j: LocalControl,
): number {
...
export function localControlNameCompare(
	i: LocalControl,
	j: LocalControl,
): number {
...
export function localControlInput(
	name: string,
	index: number,
	defaultValue: number,
): Ugen {
...
export function localControls(
	dictionary: LocalControlDictionary,
): Map<string, Signal> {
	let index = 0;
...
export function inputBranch<T>(
	input: UgenInput,
	onUgen: (aUgen: Ugen) => T,
	onNumber: (aNumber: number) => T,
	onError: () => T,
): T {
	if (isUgen(input)) {
		return onUgen(input);
...
export function inputRate(input: UgenInput): number {
	...
	return inputBranch(
		input,
		(port) => port.scUgen.rate,
		(_unusedNumber) => rateIr,
		() => -1,
	);
}
...
export function deriveRate(
	...
	if (isNumber(rateOrFilterUgenInputs)) {
		...
		return arrayMaxItem(
			arrayMap(
				inputRate,
				arrayAtIndices(inputArray, <number[]> rateOrFilterUgenInputs),
			),
		);
	}
}
...
export function mceInputTransform(
	atLeast: number,
	aSignal: Signal[],
): Signal[] {
...
export function makeUgen(
	...
	if (isArray(numChannels) || requiresMce(signalArray)) {
		...
		const expandedNumChannels = arrayExtendCyclically(
			asArray(numChannels),
			expandedSize,
		);
...
export function UnaryOpWithConstantOptimiser(
	specialIndex: number,
	input: Signal,
): Signal {
	if (isNumber(input)) {
		switch (specialIndex) {
			...
			case 16:
				return 1 / input;
			case 17:
				return scMath.midiCps(input);
			case 18:
				return scMath.cpsMidi(input);
			case 19:
				return scMath.midiRatio(input);
			case 20:
				return scMath.ratioMidi(input);
			case 21:
				return scMath.dbAmp(input);
			case 22:
				return scMath.ampDb(input);
			case 23:
				return scMath.octCps(input);
			case 24:
				return scMath.cpsOct(input);
			case 25:
...
export function UnaryOp(specialIndex: number, input: Signal): Signal {
	if (isArray(input) && arrayEvery(input, isNumber)) {
		return arrayMap(
			(item) => UnaryOpWithConstantOptimiser(specialIndex, item),
			input,
		);
	} else {
...
export function BinaryOpWithConstantOptimiser(
	specialIndex: number,
	lhs: UgenInput,
	rhs: UgenInput,
): Signal {
...
export function BinaryOp(
	...
	if (isArray(lhs) || isArray(rhs)) {
		...
		return arrayMap(
			(item) => BinaryOpWithConstantOptimiser(specialIndex, item[0], item[1]),
			<UgenInput[][]> expanded,
		);
	} else {
