...
export function riffReadChunkId(
...
export function riffReadChunkSize(
	byteArray: ArrayBuffer,
	offset: number,
): number {
...
export function riffReadChunkHeader(
	byteArray: ArrayBuffer,
	offset: number,
): RiffChunkHeader {
	return {
...
export function riffVerifyHeader(byteArray: ArrayBuffer): void {
	...
	if (header.id != 'RIFF' || header.size != (size - 8)) {
		throw new Error(
			`Invalid Riff: header.id=${header.id}, header.size=${header.size}, size=${size}`,
		);
	}
}
