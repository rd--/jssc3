...
export function treeVisit<T>(
	aTree: Tree<T>,
	visitFunction: (x: T) => void,
): void {
	if (isArray(aTree)) {
...
export function treeDepthFrom<T>(aTree: Tree<T>, depth: number): number {
	if (isArray(aTree)) {
		return arrayMaxItem(
			arrayMap((item) => treeDepthFrom(item, depth + 1), aTree),
		);
	} else {
		return depth;
	}
...
export function treeShape(aTree: Tree<unknown>): number[] {
	return isArray(aTree)
		? (arrayAppend([arraySize(aTree)], treeShape(aTree[0])))
		: [];
}
