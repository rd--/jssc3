...
export function userProgramMenuInit(
	...
	selectOnChange(
		selectId,
		(_unusedMenuElement, programName) =>
			setProgram(userPrograms.programs[programName]),
	);
	selectAddKeysAsOptions(selectId, Object.keys(userPrograms.programs));
}

export function userProgramSaveTo(
	selectId: string,
	programText: string,
	withPrompt: boolean,
): void {
	...
	if (programName) {
		...
		localStorage.setItem(
			userPrograms.storageKey,
			JSON.stringify(userPrograms.programs),
		);
...
export function userProgramReadArchive(
	inputId: string,
	selectId: string,
): void {
